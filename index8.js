...
const fs = require('fs');
var someObject = require('./sub-category/handTolld-p.json');
...
async function updatePageIndex(urls1) {
  ...
  .then(function (response) {
      var $1 = cheerio.load(response.data);
          ...
          var sku_number = $1("#product-attribute-specs-table").find("th:contains('Model')").parent().find("td")[0].children[0].data.trim();
          ...
          obj.push(objectNew);
          let data = JSON.stringify(obj, null, 2);
          fs.writeFile('Test_Measure_&_Inspect_Main_handtools.json', data, (err) => {
            if (err) throw err;
            console.log('Data written to file');
          });
